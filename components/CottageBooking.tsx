...
import { ArrowLeft, ArrowRight, CalendarDays, Check, ChevronLeft, ChevronRight, Clock, X } from 'lucide-react'
...
export default function CottageBooking({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#1E5336]/60 px-4 py-8 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative my-auto w-full max-w-2xl bg-[#FDFBF7] text-[#222222] shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          aria-label="Close booking"
          onClick={onClose}
          className="absolute right-4 top-4 z-10 border border-[#222222]/20 p-2 text-[#222222] hover:bg-[#1E5336] hover:text-[#FDFBF7]"
        >
          <X size={16} />
        </button>
          ...
          <div className="flex justify-center border-t border-[#222222]/10 px-6 py-5">
            <button
              onClick={() => {
                ...
                setNights(1)
                setEventDetails('')
                setDate(''); setTime(''); setGuests(4); setName(''); setContact(''); setPayment('GCash')
              }}
